...
public:
    ...
    int Run() {
        ...
        ImageManager& managerref = Manager;

        for (int i = 0; i < Manager.GetImageCount(); ++i) {
            threadpool.emplace_back([&managerref, i]() {
                managerref.LazyLoadImage(i);
                managerref.Compress(i);
                });
        }
...
#endif
        {
...

            {
                ...
                for (int i = 0; i < Manager.GetImageCount(); ++i) {
                    if (ImGui::Selectable(Manager.GetName(i).c_str(), selection == i, 1)) {
                        if (i != selection && selection != -1) {
                            Manager.UnloadGPU(selection);
                            Manager.Compress(selection);
                        }
                        selection = i;
                        Manager.Decompress(selection);
                        Manager.LoadGPU(selection);
                    }
                }
                ImGui::EndChild();
                ...
                {
                    if (selection != -1) {
                        ImageEntry::ImageAccess imageSelection = Manager.ReadImage(selection);
                        ImGui::LabelText("info", "File path: %s", imageSelection.source.GetFilePath().c_str());
                        ImGui::LabelText("Dimension", "%d x %d", imageSelection.source.width, imageSelection.source.height);
                        Manager.DisplayImage(selection);
                    }
                }
                ImGui::EndChild();
